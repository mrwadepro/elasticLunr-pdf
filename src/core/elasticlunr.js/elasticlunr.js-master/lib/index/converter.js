...
export default class Converter {
  constructor(inner, options) {
    if (!(inner instanceof intf))
      throw Error(
        "The object provided must be an instance of the index interface"
      );
    this.inner = inner;
              ...
              (function (eslunr) {
                ...
                idx.fields[o].settings.pipeline.forEach(fnName => {
                  ...
                  if (fn) {
                    ...
                    throw new Error(
                      "Cannot load un-registered function: " + fnName
                    );
                  }
                });
                ...
                let proxy = new Proxy(pipeline, {
                  get: (target, propKey, receiver) => {
                    ...
                    return function () {
                      return propValue.call(
                        this,
                        eslunr.tokenizer(arguments[0])
                      );
                    };
                  },
                });
  ...
  off() {
    return this.inner.emitter.removeListener.apply(
      this.inner.emitter,
      arguments
    );
  }
  ...
  elasticsearch(query) {
    if (!query || !query.query)
      throw Error("Root object must have a query element");
          ...
          query: {
            bool: {
              should: this.inner
                ...
                .filter(
                  field =>
                    field !== this.inner.getRef() &&
                    options.fields[field] &&
                    options.fields[field].boost > 0
                )
                .map(field => {
                  return {
                    match: {
