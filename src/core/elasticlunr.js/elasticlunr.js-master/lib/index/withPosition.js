...
class Token {
  ...
  getPosition() {
    if (this.metadata.start && this.metadata.end)
      return [this.metadata.start, this.metadata.end];
    return null;
  }
...
class Field {
  ...
  highlight(id, highlightPositions) {
    ...
    let sortedPositions = highlightPositions
      .sort((a, b) => a[0] - b[0])
      .reduce((current, pos) => {
        if (!current.length) {
          current.push(pos);
    ...
    return (startTag, endTag) => {
      ...
      for (let i = 0; i < sortedPositions.length; i++) {
        components.push(
          str.substring(
            i === 0 ? 0 : sortedPositions[i - 1][1],
            sortedPositions[i][0]
          )
        );
        components.push(startTag);
        components.push(
          str.substring(sortedPositions[i][0], sortedPositions[i][1])
        );
        components.push(endTag);
      }
      if (sortedPositions.length > 0) {
        components.push(
          str.substring(sortedPositions[sortedPositions.length - 1][1])
        );
      }
  ...
  terms(query) {
    ...
    query.terms.forEach(term => {
      if (term instanceof RegExp) {
        ...
        terms
          .filter(idxTerm => term.test(idxTerm))
          .map(matchedTerm => {
            let ids = Object.keys(this._terms[matchedTerm]);

            (query.docs
              ? ids.filter(i => query.docs.indexOf(i) >= 0)
              : ids
            ).forEach(i => {
              ...
              matchingDocs[i].push({
                ...
                norm: this._flnorm,
                content:
                  this._store && this._documents[i] ? this._documents[i] : null,
              });
            });
          });
    ...
    return Object.entries(matchingDocs)
      .filter(([docId, content]) => {
  ...
  update(documents) {
    var toRemove = documents
      .filter(doc => this._ids.indexOf(doc.id) >= 0)
      .map(doc => doc.id);
  ...
  recalculateIDF() {
    ...
    Object.keys(this._terms).forEach(token => {
      this._idf[token] =
        1 +
        Math.log10(
          this._ids.length / (Object.keys(this._terms[token]).length + 1)
        );
    });
  }
  add(documents) {
    ...
    for (let o of documents) {
      if (this._ids.indexOf(o.id) >= 0)
        throw Error(`Document id ${o.id} already exists in the index`);
      updated.push(o.id);
  ...
  analyze(str, options) {
    if (options && options.isQuery && this.queryPipeline)
      return this.queryPipeline.run(str);
...
class Index extends IdxInterface {
  ...
  _load(idxData) {
    ...
    idxData.fields.forEach(field => {
      ...
      this.addField(field.name, field.settings || {});

      if (
        field.settings &&
        field.settings.store &&
        field.settings.store.documents
      ) {
  ...
  toJSON() {
    ...
    Object.entries(this._fields).forEach(([fieldName, field]) => {
      ...
      field.allTokens().forEach(term => {
        ...
        idxJson[term.term].push([
          fieldID,
          { tf: term.tf, idf: term.idf, p: term.terms },
        ]);
      });
    });
  ...
  remove(docIDs, muteEvent) {
    var updatedIds = Object.values(this._fields).reduce(
      (updatedIds, field) => union(updatedIds, field.remove(docIDs)),
      []
    );
